...
print("\n=== MISSING VALUES ===")
print(
    f"Target usgs_level: {river_target_st_louis['usgs_level'].isna().sum()} missing ({river_target_st_louis['usgs_level'].isna().sum() / len(river_target_st_louis) * 100:.2f}%)")
print(
    f"Grafton level: {river_upstream_grafton['grafton_level'].isna().sum()} missing ({river_upstream_grafton['grafton_level'].isna().sum() / len(river_upstream_grafton) * 100:.2f}%)")
print(
    f"Hermann level: {river_upstream_hermann['hermann_level'].isna().sum()} missing ({river_upstream_hermann['hermann_level'].isna().sum() / len(river_upstream_hermann) * 100:.2f}%)")
