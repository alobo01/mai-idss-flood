    ...
    **Current State**: 80% architecture, 20% functionality. The scaffolding is complete but page components are mostly placeholders.
    ...
    - **API**: Express.js server backed by PostgreSQL (port 18080)
    ...
    npm install
    npm run dev:full          # Start backend API (port 18080) and frontend (port 5173)
    npm run dev               # Frontend only (port 5173)
    npm run api               # API only (port 18080)
    ...
    **Application URLs**:
    ...
    api/              # Express server code
    ```

    ### State Management
    - **AppContext**: Global state (role, zone selection, theme)
    - **React Query**: Data fetching from API (configured but not used)
    ...
    #### API (Port 18080)
    Express server backed by PostgreSQL/PostGIS:
    ...
    - `GET /api/admin/resources/*` - Admin resource dashboards
    ...
    3. **Data**: Use API endpoints via React Query hooks
    ...
    - **API Container**: Express serving JSON data
    ...
    - API can be replaced with real backend services
    ...
    2. **Data Integration** - Connect UI components to the API
    ...
    ### API Integration
    - API runs on port 18080 in development
